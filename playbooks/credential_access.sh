#!/bin/bash
# Credential Access Playbook - ENHANCED with full output
# =======================================================
# STANDALONE - does NOT require discovery to run first
# Shows actual extracted credentials, file paths, LSASS info
...
main() {
    echo ""
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║         Credential Access Attack Playbook                    ║"
    echo "║         MITRE ATT&CK: Credential Access (TA0006)             ║"
    ...
    echo ""
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║           EXECUTING CREDENTIAL ACCESS ATTACKS                ║"
    echo "╚══════════════════════════════════════════════════════════════╝"

    execute_and_show "T1555.003" "Browser Credentials — Extract Edge saved passwords"
    execute_and_show "T1552.001" "Credentials in Files — Search for cred files"
    execute_and_show "T1003.001" "LSASS Memory — Enumerate LSASS process"
    ...
    log_success "Credential Access playbook completed!"
    echo ""
}
